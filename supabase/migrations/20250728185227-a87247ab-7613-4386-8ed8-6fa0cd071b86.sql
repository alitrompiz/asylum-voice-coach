-- Drop and recreate the admin function with correct types
DROP FUNCTION IF EXISTS public.get_all_users_admin(text, text, integer, integer);
...
RETURNS TABLE(
    user_id uuid,
    email character varying,  -- Fixed: match the actual auth.users email type
    display_name text,
    is_banned boolean,
